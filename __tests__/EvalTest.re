...
describe("Simple scriplet eval", () => {
  ...
  test("assign a constant to x", () =>
    expect(createEvalFunction("x = 2")(1.0)) |> toEqual({...Scriptlet.initialState, x: 2.0, y: 0.0}))

  test("assign t to y", () =>
    expect(createEvalFunction("y = t")(7.0)) |> toEqual({...Scriptlet.initialState, x: 0.0, y: 7.0}))
