...
let getLayoutGridInfo = (config: config): option(layoutGridInfo) => {
...
let make = (~debugModes: DebugMode.debugModes, ~infoCallback: layoutGridInfoCallback) => {
  ...
  <div className="noteInfoGridProbeStrip">
    <div className="noteInfoGridProbeRow">
      <div id="probe0" className="noteInfoGridProbeCell"> {React.string("0")} </div>
      <div id="probe1" className="noteInfoGridProbeCell"> {React.string("1")} </div>
    </div>
  </div>;
};